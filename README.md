...
| Storage  | SQLite via Node's built-in `node:sqlite` — `server/data/devtrack.db` |
| Auth     | Username + password (scrypt), or Google sign-in; cookie sessions |

There is no database server and no native module to compile: `npm install` and
go (Node 22.13+ for `node:sqlite`). The database holds three tables — `users`,
`sessions`, and `workspaces` (one row per account with its applications,
resumes and settings) — all set up in `server/db.js`.
...
Open the app and create an account, or continue with Google. **Every account
starts with an empty workspace** and only ever sees its own data.

To try the app with sample data, load the demo set into an account:

```bash
npm run seed -- <username>           # only if that workspace is empty
npm run seed -- <username> --force   # replace what's there
```

### Google sign-in

Optional; username/password works without it.

1. In Google Cloud Console → APIs & Services → Credentials, create an **OAuth
   client ID** of type *Web application*.
2. Add `http://localhost:5173` (and `http://localhost:5174` if you use
   `npm start`) under **Authorized JavaScript origins**.
3. Copy `.env.example` to `.env`, set `GOOGLE_CLIENT_ID`, and restart the server.

The browser gets an ID token from Google; the server checks it with Google and
requires its audience to match your client ID before signing anyone in.
...
| GET    | `/api/auth/config`                            | Google client ID for the login page  |

Everything except `/api/health` and `/api/auth/*` needs a session and acts on
the signed-in user's workspace only.
...
server/
  ...
  db.js               SQLite schema; per-user workspace read / serialised write
  auth.js             Accounts, password hashing, sessions, Google verification
  model.js            Shared enums, validators, promise-aware Router
  seed.js             Demo data, loaded into an account on request
...
src/
  ...
  pages/              Login · Applications · ResumeWorkspace · Analytics · Settings
```
